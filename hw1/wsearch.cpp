...
bool readGrid(istream& instream, vector<vector<char> >& grid )
{
  ...
  while(iss >> current){
    col++;
    grid[0].push_back(current);
  }
  if(col == 0){
    success = false;
  }
  string newline;
  while(getline(instream, newline)){
    if(line.empty()){
      break;
    } 
    row++;
...
{
  /*** You complete ***/
  if((unsigned)currLoc.row >= grid.size() || (unsigned)currLoc.col >= grid[0].size()){
    return false;
  }
  char current = grid[currLoc.row][currLoc.col];
  if(currWordIndex < word.length() && current == word[currWordIndex]){
